...
type family TyEq t1 t2 where
    TyEq x x = TTrue
    TyEq x y = TFalse


-- | 'MemberEq m n' is true iff the members 'm' and 'n' have the same types.
type family MemberEq m n :: TBool where
    MemberEq TNil       TNil       = TTrue
    MemberEq TNil       (n ::: ns) = TFalse
    MemberEq (m ::: ms) TNil       = TFalse
    MemberEq (m ::: ms) (n ::: ns) = TAnd (TyEq m n) (MemberEq ms ns)

-- | @'TyElem' t ts@ is true iff the type @t@ is present in the list @ts@
--   (containing coded type representations).
type family    TyElem t1 ts :: TBool where
    TyElem t1 TNil        = TFalse
    TyElem t1 (t ::: ts)  = TOr (TyEq t1 t) (TyElem t1 ts)
...
type family FilterBestMembers as ms ns where
    FilterBestMembers as ms TNil       = TNil
...
--
type family IsBestMember as ms n :: TBool where
    IsBestMember as TNil       n = TTrue
    IsBestMember as (m ::: ms) n =
...
type family    IsApp  as          ps :: TBool where
    IsApp  TNil        TNil       = TTrue
    IsApp  TNil        (p ::: ps) = TFalse -- different lengths
    IsApp  (a ::: as)  TNil       = TFalse -- different lengths
    IsApp  (a ::: as)  (p ::: ps) = TAnd (ConvertsTo a p) (IsApp as ps)
...
--
type family IsBetterMember as ps qs :: TBool where
    IsBetterMember as ps qs =
        TAnd (AnyBetterConv as ps qs) 
            (TNot (AnyBetterConv as qs ps))


type family    AnyBetterConv  as          ss          ts :: TBool where
    AnyBetterConv  TNil        TNil        TNil        = TFalse
    AnyBetterConv  (a ::: as)  (s ::: ss)  (t ::: ts)  = TOr (IsBetterConv a s t) (AnyBetterConv as ss ts)


type family    IsBetterConv t t1 t2 :: TBool where
    IsBetterConv t t1 t2 = TOr  (TAnd (TyEq t t1) (TNot (TyEq t t2)))
...
--   to @t@ (ref 6.1).
type family    ConvertsTo t1 t2 :: TBool where
    ConvertsTo t1 t2 =
        (TOr  (TyEq t1 t2)
        (TOr  (TAnd (IsPrim t1)           (TyEq t2 (Obj Object_)))
        (TOr  (TAnd (TyEq t1 Int32)       (TyEq t2 Double))
...
type family    IsSubtypeOf t1 t2 :: TBool where
    IsSubtypeOf t1 t2  = TOr (TyEq t1 t2)
        (TyElem t2 (SupertypesOf t1))

type family    IsArr t :: TBool where
    IsArr t = TAnd (TNot (TyEq t (Obj Array_)))
